...
quat_test_lll_lideal_lideal_mul_reduced()
{
    ...
    quat_lattice_mul(&i1.lattice, &i1.lattice, &i2.lattice, &alg, &i1.norm, &i2.norm);
...
quat_test_lll_lideal_prime_norm_reduced_equivalent()
{
    ...
    ibz_t norm_ro, norm_ro2;
    ibz_init(&norm_ro);
    ibz_init(&norm_ro2);
    ibz_set(&norm_ro, 1);
    ibz_set(&norm_ro2, 1);
    quat_lattice_mul(&(lideal2.lattice), &ro, &ro2, &alg, &norm_ro, &norm_ro2);
    ...
    ibz_finalize(&n);
    ibz_finalize(&d);
    ibz_finalize(&norm_ro);
    ibz_finalize(&norm_ro2);
    ibz_mat_4x4_finalize(&red);
    ibz_mat_4x4_finalize(&gram);
    quat_alg_elem_finalize(&gen);
    quat_left_ideal_finalize(&lideal1);
