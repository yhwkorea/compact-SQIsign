...
quat_test_lattice_mul(void)
{
    ...
    // Compute norms as determinants
    ibz_mat_4x4_inv_with_det_as_denom(NULL, &norm_lat1, &(lat1.basis));
    ibz_abs(&norm_lat1, &norm_lat1);
    ibz_mat_4x4_inv_with_det_as_denom(NULL, &norm_lat2, &(lat2.basis));
    ibz_abs(&norm_lat2, &norm_lat2);

    quat_lattice_mul(&prod, &lat1, &lat2, &alg, &norm_lat1, &norm_lat2);
    ...
    // Compute norms as determinants (recompute since lattices changed)
    ibz_mat_4x4_inv_with_det_as_denom(NULL, &norm_lat1, &(lat1.basis));
    ibz_abs(&norm_lat1, &norm_lat1);
    ibz_mat_4x4_inv_with_det_as_denom(NULL, &norm_lat2, &(lat2.basis));
    ibz_abs(&norm_lat2, &norm_lat2);
    ...
    // norm_lat2 already computed, reuse it for lat2 * lat2
    quat_lattice_mul(&lat2, &lat2, &lat2, &alg, &norm_lat2, &norm_lat2);
    res = res || (!ibz_mat_4x4_equal(&(lat2.basis), &(cmp.basis)));
    res = res || ibz_cmp(&(lat2.denom), &(cmp.denom));
    ...
    quat_lattice_finalize(&cmp);
    quat_alg_finalize(&alg);
    ibz_finalize(&norm_lat1);
    ibz_finalize(&norm_lat2);
    return (res);
}
