...
static int lattice_closed_under_mul(const quat_lattice_t *L, const quat_alg_t *alg)
{
    ibz_vec_4_t a, b, p;
    quat_alg_elem_t e;
    ...
    // e = (p / (denom^2))
    ibz_mul(&e.denom, &L->denom, &L->denom);

    for (int k = 0; k < 4; k++) {
        ibz_vec_4_copy_ibz(&a,
            &(L->basis[0][k]), &(L->basis[1][k]), &(L->basis[2][k]), &(L->basis[3][k]));

        for (int i = 0; i < 4; i++) {
            ibz_vec_4_copy_ibz(&b,
                &(L->basis[0][i]), &(L->basis[1][i]), &(L->basis[2][i]), &(L->basis[3][i]));

            quat_alg_coord_mul(&p, &a, &b, alg);

            for (int j = 0; j < 4; j++) {
                ibz_copy(&(e.coord[j]), &(p[j]));
            }

            if (!quat_lattice_contains(NULL, L, &e)) {
...
{ // check the order is maximal and i,j generate a suborder
    ...
    quat_lattice_t lat;
    quat_lattice_t test;
    // ibz_t det_lat, det_order, norm_lat, norm_order, tmp, rem;
    ibz_vec_4_init(&ij);
    ...
    // // Compute norms using formula: nrd(I) = sqrt(det(I) / det(O))
    // // For lat (which is a sublattice of order)
    // ibz_mat_4x4_inv_with_det_as_denom(NULL, &det_lat, &(lat.basis));
    // ibz_abs(&det_lat, &det_lat);
    // ibz_mat_4x4_inv_with_det_as_denom(NULL, &det_order, &(params->order->order.basis));
    // ibz_abs(&det_order, &det_order);


    // /*-----------------------*/

    // // norm_lat = sqrt(det_lat / det_order)
    // ibz_div(&tmp, &rem, &det_lat, &det_order);
    // ibz_sqrt(&norm_lat, &tmp);

    // /*-----------------------*/

    // // For order itself, norm = 1 when viewed as ideal of itself
    // ibz_set(&norm_order, 1);

    // quat_lattice_mul(&test, &lat, &lat, alg, &norm_lat, &norm_lat);
    // res = res && quat_lattice_inclusion(&test, &lat);
    res = res && lattice_closed_under_mul(&lat, alg);


    // quat_lattice_mul(&test, &(params->order->order), &(params->order->order), alg, &norm_order, &norm_order);
    // res = res && quat_lattice_inclusion(&test, &(params->order->order));
    res = res && lattice_closed_under_mul(&(params->order->order), alg);
    ...
    // ibz_finalize(&det_lat);
    // ibz_finalize(&det_order);
    // ibz_finalize(&norm_lat);
    // ibz_finalize(&norm_order);
    // ibz_finalize(&tmp);
    // ibz_finalize(&rem);
    return (res);
}
