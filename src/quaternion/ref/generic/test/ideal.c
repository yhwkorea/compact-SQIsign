...
quat_test_lideal_conj_without_hnf(void)
{
    ...
    quat_lattice_mul(&test, &(lideal.lattice), &(conj.lattice), &alg, &(lideal.norm), &(conj.norm));
    ibz_copy(&(cmp.denom), &(o.denom));
    ibz_mat_4x4_scalar_mul(&(cmp.basis), &n, &(o.basis));
    res = res || !quat_lattice_equal(&test, &cmp);
    quat_lattice_mul(&test, &(conj.lattice), &(lideal.lattice), &alg, &(conj.norm), &(lideal.norm));
...
quat_test_lideal_inverse_lattice_without_hnf(void)
{
    ...
    quat_lattice_mul(&prod, &(lideal.lattice), &inv, &alg, &(lideal.norm), &(lideal.norm));
...
quat_test_lideal_right_transporter(void)
{
    ...
    // trans = I1^{-1} * I2, norm of trans = nrd(I1) * nrd(I2)
    quat_lideal_right_transporter(&trans, &lideal1, &lideal2, &alg);
    ibz_mul(&norm_trans, &(lideal1.norm), &(lideal2.norm));
    quat_lattice_mul(&prod, &(lideal1.lattice), &trans, &alg, &(lideal1.norm), &norm_trans);
    res = res || !quat_lattice_inclusion(&prod, &(lideal2.lattice));
    // trans = I2^{-1} * I1, norm of trans = nrd(I2) * nrd(I1)
    quat_lideal_right_transporter(&trans, &lideal2, &lideal1, &alg);
    ibz_mul(&norm_trans, &(lideal2.norm), &(lideal1.norm));
    quat_lattice_mul(&prod, &(lideal2.lattice), &trans, &alg, &(lideal2.norm), &norm_trans);
...
quat_test_lideal_right_order(void)
{
    ...
    quat_alg_elem_init(&gen);
    ibz_init(&norm);
    ibz_init(&norm_rorder);
    ibz_set(&norm_rorder, 1);
    ...
    quat_lattice_mul(&prod, &rorder, &rorder, &alg, &norm_rorder, &norm_rorder);
    res = res || !quat_lattice_inclusion(&prod, &rorder);
    // test it is right order of ideal
    quat_lattice_mul(&prod, &(lideal.lattice), &rorder, &alg, &(lideal.norm), &norm_rorder);
